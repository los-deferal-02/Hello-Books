  ...
  () => {
    ...
    it('respond with error 404 if user has no favourite author', async () => {
      ...
      expect(res.body.data.author).to
        .equal('You have not yet favourited any author');
    });
    ...
    it('respond with 200 when author is deleted from favourite list',
      async () => {
        const res = await chai.request(app)
    ...
    async () => {
      ...
      expect(res).to.have.status(404);
      expect(res.body.data.author).to
        .equal('Author Not Found in your Favourite List');
    });
  });
});
